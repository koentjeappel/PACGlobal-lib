# Contributing to pacglobal-lib

We're working hard to make *pacglobal-lib* the most powerful JavaScript library for working with PACGlobal. Our goal is to have *pacglobal-lib* be a library that can be used by anyone interested in PACGlobal, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.pacglobal.org/forum (Support Forum)
...
These are some global design goals in pacglobal-lib that any change must adhere.
...
When possible, pacglobal-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of pacglobal-lib methods. For example, calling `JSON.stringify` on an pacglobal-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'pacglobal' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the pacglobal object.

DO:
```javascript
var pacglobal = require('../');
var PublicKey = pacglobal.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add PACGlobal git@github.com:koentjeappel/PACGlobal-lib.git
git pull --rebase PACGlobal master
```
...
```
Finally go to [github.com/koentjeappel/PACGlobal-lib](https://github.com/koentjeappel/PACGlobal-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of pacglobal-lib.

If you have any questions feel free to post them to
[github.com/koentjeappel/PACGlobal-lib/issues](https://github.com/koentjeappel/PACGlobal-lib/issues).
